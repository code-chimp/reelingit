...
// AccountRepository is an AccountStorage implementation backed by
// PostgreSQL.
...
// NewAccountRepository creates an AccountRepository that queries db,
// logging query failures to log.
...
// storing password as a bcrypt hash. It returns ErrRegistrationValidation
// if any field is empty, or ErrUserAlreadyExists if a user with email
// already exists.
...
// Authenticate verifies email and password against the stored bcrypt hash
// and, on success, updates the user's last-login time (a failure to update
// last-login does not fail authentication). It returns
// ErrAuthenticationValidation if the credentials are missing, the email is
...
// GetAccountDetails returns the user with the given email, including their
// favorite and watchlist movies. It returns ErrUserNotFound if no
// non-deleted user has that email.
...
// "watchlist" collection (collection must be one of those two values).
// It is idempotent: if the movie is already in the collection, it returns
// (true, nil) without inserting a duplicate row. It returns ErrUserNotFound
// if user's email doesn't match a non-deleted account.
...
var (
	// ErrRegistrationValidation is returned by Register when name, email,
	// or password is empty.
	ErrRegistrationValidation = errors.New("registration failed")

	// ErrAuthenticationValidation is returned by Authenticate when the
	// credentials are missing, the email is unknown, or the password
	// doesn't match.
	ErrAuthenticationValidation = errors.New("authentication failed")

	// ErrUserAlreadyExists is returned by Register when a user with the
	// given email already exists.
	ErrUserAlreadyExists = errors.New("user already exists")

	// ErrUserNotFound is returned by GetAccountDetails and SaveCollection
	// when no non-deleted user matches the given email.
